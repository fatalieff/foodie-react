    ...
    <>
      <div className="pt-6 sm:pt-8 lg:pt-12">
        <div className="flex flex-col lg:flex-row items-center gap-6 lg:gap-8">
          <div className="w-full lg:w-1/2 text-center lg:text-left">
            <h1 className="nunito text-3xl sm:text-4xl md:text-5xl lg:text-[61px] font-bold leading-tight text-[#2D2D2D]">
              Desire{" "}
              <span className="inline-block bg-gradient-to-r from-[#FF9E0C] to-[#D58000] text-white px-3 sm:px-4 py-1 rounded-full -rotate-[11deg] shadow-[0_6px_20px_rgba(213,128,0,0.35)] text-2xl sm:text-3xl lg:text-[61px]">
                Food
              </span>
              <br />
              for Your Taste
            </h1>
            <p className="nunito text-base sm:text-lg lg:text-[25px] text-[#666666] leading-relaxed mt-3 sm:mt-4 max-w-xl mx-auto lg:mx-0">
        ...
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mt-8 sm:mt-12 lg:mt-16">
          {services.map((item, i) => (
            <div key={i} className={serviceCardClass}>
              ...
              <span className="nunito font-semibold text-xl sm:text-2xl lg:text-[30px] text-[#2D2D2D] text-center">
                {item.title}
              </span>
              <p className="nunito font-medium text-sm sm:text-base lg:text-[20px] text-[#666666] text-center">
                Contrary to popular belief, Lorem Ipsum is not simply random text
              </p>
              <NavLink to="/LearnMore" className="mt-2 sm:mt-4">
                <span className="text-[#2D2D2D] text-base sm:text-lg lg:text-[20px] font-bold nunito hover:text-[#F03328] transition-colors">
                  Learn More
                </span>
              </NavLink>
            </div>
      ...
      <div className="mt-12 sm:mt-16 lg:mt-24 flex flex-col lg:flex-row items-stretch gap-8 lg:gap-12 relative z-[1]">
        ...
        <div className="w-full lg:w-1/2 min-h-0 flex flex-col justify-between gap-6">
          <span className="nunito font-bold text-2xl sm:text-3xl lg:text-[42px] text-center shrink-0 bg-gradient-to-br from-[#2d2d2d] to-[#5c4033] bg-clip-text text-transparent">
              ...
              <div key={item.title} className={featureCardClass}>
                <div className="flex items-center gap-3 sm:gap-4">
                  ...
                  <div className="flex flex-col gap-1 min-w-0">
                    <span className="nunito font-semibold text-lg sm:text-xl lg:text-[22px] leading-tight">
                      {item.title}
                    </span>
                    <p className="nunito font-medium text-sm sm:text-[15px] text-[#404040] leading-snug">
                      {item.text}
                    </p>
                  </div>
                </div>
